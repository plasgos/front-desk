  ...
  return (
    ...
    >
      {progressPackage.map((progress, index) => {
        const isActive = pathname === `/package-history${progress.url}`;

        return (
          <Link
            key={index}
            style={{ color: "black" }}
            to={`/package-history${progress.url}`}
          >
            <div
              style={{ width: 250 }}
              ...
              <div className="card-body">
                <div className="d-flex justify-content-between align-items-center">
                  <div className="icon-hover">
                    {statusToIcon[progress.status]}
                  </div>
                  <div style={{ fontSize: 36 }} className="font-weight-bold">
                    {progress.value}
                  </div>
                </div>
                <div
