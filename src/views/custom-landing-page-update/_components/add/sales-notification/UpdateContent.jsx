...
export const UpdateContent = ({
  ...
  const [price, setPrice] = useState(
    isEditingContent ? currentContent?.price : 70000
  );

  const [productName, setProductName] = useState(
    isEditingContent ? currentContent?.productName : "Topi"
  );

  const [nameValue] = useDebounce(name, 300);
  const [locationValue] = useDebounce(location, 300);
  const [timeValue] = useDebounce(time, 300);
  const [productNameValue] = useDebounce(productName, 300);
  ...
  useEffect(() => {
    ...
    if (priceValue !== currentContent?.price) {
      handleChangeContent("price", priceValue);
    }

    if (productNameValue !== currentContent?.productName) {
      handleChangeContent("productName", productNameValue);
    }

    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [nameValue, locationValue, timeValue, priceValue, productNameValue]);
