...
import { checkCosts, resetCheckCosts } from '../../../../redux/modules/shipping/reducer';
...
const Costs = () => {
  ...
  const { sender, receiver, detail, items, shipping, item_value } = useSelector(state => state.package);
  ...
  const isValid = () => {
    ...
    return checkObjectFilled(obj) && detail.weight > 0 && detail.length > 0 && detail.width > 0 && detail.height > 0
  }
  ...
  const onSetCod = (value) => {
    ...
    if(shipping.courier && shipping.courier.costs && value){
      payload.cod_setting = {
        ...
        minimum_cod_fee: Number(shipping.courier.costs[0].setting.minimum_cod_fee),
      }
      payload.cod_fee = codFee(item_value, Number(shipping.courier.costs[0].setting.cod_fee), Number(shipping.courier.costs[0].setting.minimum_cod_fee))
    }else{
      payload.cod_fee = 0;
    }
  ...
  useEffect(()=>{
    if(cost.data){
      const costs = cost.data.filter(item => item.costs.some(val => val.cod === shipping.cod && val.drop === shipping.drop));
      const arr = filterCostByGroup(costs, groupNames).length > 0 ? filterCostByGroup(costs, groupNames)[0] : []
      setGroup(arr)
    }
    return () => {}
  ...
  return (
    <div>
      ...
      {
        filterCostByGroup(cost.data.filter(item => item.costs.some(val => val.cod === shipping.cod && val.drop === shipping.drop)), groupNames).length > 0 ? (
          <>
            <div className="mb-2 d-flex align-items-center">
              {
                filterCostByGroup(cost.data.filter(item => item.costs.some(val => val.cod === shipping.cod && val.drop === shipping.drop)), groupNames).map((item, i) => (
                  <CButton key={i} size="sm" shape="pill" variant="outline" color="primary" className={`mr-2 ${item.service === group.service ? "bg-primary text-white" : ""}`} onClick={()=>setGroup(item)}>
                    <div><b>{item.service}</b></div>
                  </CButton>
                ))
              }
            </div>
            {
              group.courier && group.courier.length > 0 && (
                <div>
                  {
                    group.courier ? group.courier.map((item, i) => (
                      <div
                        style={{ cursor: "pointer" }}
                        key={item.id}
                        className={`card px-3 py-2 shadow-sm ${item.costs[0].id === shipping.service_type_id  ? "border border-primary" : ""} `}
                        onClick={() => onSetCourier(item)}
                      >
                        <div className="d-flex justify-content-between align-items-center">
                          <div>
                            <div className="font-weight-bold mb-2">{item.name} <span className="text-muted">{item.costs[0].service_name}</span></div>
                            <div>
                              <div className="font-weight-bold mb-2 text-success">
                                <span>{formatPrice(Number(item.costs[0].cost))}</span>
                              </div>
