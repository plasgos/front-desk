...
import { CButton, CSwitch,CCard, CCardBody, CTooltip, CAlert, CLink } from "@coreui/react";
...
import { setShipping, setInsurance } from '../../../../redux/modules/package/reducer';
...
const Insurance = () => {
  ...
  const services_express_ids = Array.from({ length: 12 }, (_, index) => index + 4);
  ...
  const insurance_express = () => {
    if(shipping.courier && services_express_ids.includes(shipping.courier.id) && shipping.courier.costs){
      let total = subTotal()
      let insurance_fee = (Number(shipping.courier.costs[0].setting.insurance_fee) * total);
      if(shipping.courier.costs[0].setting.insurance_add_cost){
        insurance_fee = (insurance_fee + Number(shipping.courier.costs[0].setting.insurance_add_cost))
      }
      return {
        insurance_fee,
        insurance_setting: {
          insurance_fee: Number(shipping.courier.costs[0].setting.insurance_fee),
          insurance_add_cost: Number(shipping.courier.costs[0].setting.insurance_add_cost)
        }
      }
    }
  }
    ...
    try{
      if(checked && shipping.courier){
        ...
        if(services_express_ids.includes(shipping.courier.id)){
          payload.insurance_fee = roundUpToNearestHundred(insurance_express().insurance_fee)
        }
      }
      if(!checked){
        ...
        <CCardBody className="py-2 d-flex justify-content-between align-items-center">
          <div>
            <div className="d-flex justify-content-start align-items-center">
              <div><b>{(shipping.courier && services_instant_ids.includes(shipping.courier.id)) || subTotal() >= 500000? 'Wajib Asuransi' : 'Asuransi Pengiriman'}</b></div>
              <div className="text-dark ml-2">
                <CTooltip content="Pesanan kamu mendapat perlindungan asuransi pengiriman. Harga asuransi yang berlaku sesuai dengan ketentuan dari masing-masing ekspedisi yang kamu pilih." placement="bottom">
                  <IoInformationCircle style={{fontSize: 16, cursor:'pointer'}} />
                </CTooltip>
              </div>
            </div>
            {
              shipping.courier && services_express_ids.includes(shipping.courier.id) && shipping.courier.costs && (
                <div className="text-dark" style={{fontSize:12}}>Biaya asuransi <b>{formatPrice(roundUpToNearestHundred(insurance_express().insurance_fee))}</b></div>
              )
            }
          </div>
          <CSwitch color="primary" shape="pill" disabled={(shipping.courier && services_instant_ids.includes(shipping.courier.id)) || subTotal() >= 500000} checked={shipping.insurance} onClick={()=>onCheckedInsurance(!shipping.insurance)} style={{fontSize: 24}} />
        </CCardBody>
      </CCard>
    </>
  )
...
export default ({history}) => {
  ...
  const { warehouse_id, shipping_cost, insurance_fee, cod_fee, item_value, shipping, items, detail, receiver, sender } = useSelector((state) => state.package);
  ...
  return (
    <div className="mb-3">
      ...
      <div className="pt-3 card p-3 shadow-sm">
        ...
        <div>
          <div className="d-flex justify-content-between align-items-center">
            ...
            <div className="font-weight-bold">{shipping.name && shipping.name.length > 0 ? shipping.name : "-"}</div>
          </div>
          <div className="d-flex justify-content-between align-items-center">
          ...
          <div className="mt-3">
            <CAlert color="info">
              <div className="d-flex justify-content-between align-items-center">
                ...
                <div><CLink href="#">Details</CLink></div>
              </div>
            </CAlert>
          </div>
        </div>
      ...
      <div className="d-flex align-items-center justify-content-end">
        <CButton className="btn-outline-primary mr-2" style={{width: 100}} onClick={() => history.goBack()}><span className="ont-weight-bold">Batal</span></CButton>
