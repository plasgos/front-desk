        ...
        <div style={{ gap: 20 }} className="d-flex ">
          <div className="rounded shadow-sm border p-2">
            <img
              ...
              src={defaultImgSquare200(detail)}
              alt="display-product"
            />
          </div>
          <div>
      ...
      <CCardBody className="p-2">
        <div className="d-flex justify-content-start align-items-start">
          ...
          <div className="ml-3">
            ...
            <div className="mb-1" style={{ fontSize: 12 }}>
              {item.quantity} barang ({item.product.weight * item.quantity}{" "}
              gram)
            </div>
        ...
        <div className="d-flex align-items-center justify-content-between">
          ...
          >
            Hapus Produk{" "}
            <IoTrashOutline style={{ fontSize: 18 }} className="ml-2" />
          </CButton>
          <div className="d-flex align-items-center justify-content-end">
            <div className="mr-2">
              <div>Subtotal</div>
            ...
            <div className="d-flex align-items-center">
              <div
                ...
                onClick={() => onChangeQtyMinus(qty)}
              >
                <IoRemoveCircleOutline
                  style={{ fontSize: 24 }}
                  color={item.quantity === 1 ? "#dae2ed" : "#fa541c"}
                />
              </div>
              <div className="px-2">
                <CInput
              ...
              <div
                style={{ cursor: "pointer" }}
                onClick={() => onChangeQtyAdd(qty)}
              >
                <IoAddCircleOutline style={{ fontSize: 24 }} color="#FA541C" />
              </div>
            </div>
          </div>
        </div>
...
const ListSelectedProducts = () => {
  ...
  const weight = items.reduce(
    (acc, val) => acc + val.product.weight * val.quantity,
    0
  );
  return (
    <div style={{ flex: 1 }}>
      ...
      <div className="modal-overflow vertical-scrolling-menu">
        {items.length > 0 ? (
          <>
            {items
              .map((item, i) => (
                <CardSelected key={item.product_id} item={item} />
              ))
              .reverse()}
            <div>
              <div>
...
export const CatalogProductsModal = () => {
  ...
  const filteredNotInclude = data.filter(
    (product) => !items.some((item) => product.id === item.product_id)
  );

  const [modal, setModal] = useState(false);
  const toggle = async () => {
    if (!modal === false) {
      const total_item_value = await items.reduce(
        (acc, val) => acc + val.product.price * val.quantity,
        0
      );
      const total_weight = await items.reduce(
        (acc, val) => acc + val.product.weight * val.quantity,
        0
      );
      ...
      <CModal size="xl" centered show={modal} onClose={toggle}>
        ...
        <CModalBody style={{ gap: 20 }} className="d-flex">
          <div style={{ flex: 1 }}>
            ...
            <div className="modal-overflow vertical-scrolling-menu">
              {data.length > 0 ? (
                filteredNotInclude.map((product, i) => (
                  <div key={i}>
                    <CardProduct detail={product} />
                  </div>
                ))
              ) : (
                <div>Kosong</div>
